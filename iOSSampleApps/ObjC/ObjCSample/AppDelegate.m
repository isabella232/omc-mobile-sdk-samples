...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Flush the test cache. This is for development purposes only and is only available in staging mode.
    // Flushing the test cache forces the application to query the Optimize server at every startup.
    
    [[WTOptimizeManager sharedManager] flushTestCache];
...
- (void)session:(WCSession *)session didReceiveMessage:(NSDictionary<NSString *,id> *)message
{
    ...
    if ([collector saveWatchEvent:message]) {
        return;
    }
    
    // Your code here for handling the receipt of a message from the watch.
